...
# ----------------------------------------------
# 5          | Calibration errors | 2 sources at -20° and 20°, SNR_1 is 15dB, SNR_1 is -15dB, AWGN, steering vectors perturbed

def demo_five(M = 8,d = 0.5,N = 100, figName = None):        
    inc_ang_deg = [-20, 20]
    ...
    array_errors = np.random.uniform(-0.2, 0.2, M)  # Small random errors in positions
    perturbed = d + array_errors
    A = ula_steering_matrix(M,perturbed[:,np.newaxis],thetas_rad) # (M x K)
